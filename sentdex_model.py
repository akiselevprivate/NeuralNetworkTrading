...
def preprocess_df(df):
    ...
    for col in df.columns:  # go through all of the columns
        if col != "target":  # normalize all ... except for the target itself!
            df[col] = df[
                col
            ].pct_change()  # pct change "normalizes" the different currencies (each crypto coin has vastly diff values, we're really more interested in the other coin's movements)
            df.dropna(inplace=True)  # remove the nas created by pct_change
            df[col] = preprocessing.scale(df[col].values)  # scale between 0 and 1.

    df.dropna(inplace=True)  # cleanup again... jic.

    df.to_csv("sentdex.csv")
    ...
    lower = min(len(buys), len(sells))  # what's the shorter xlength?
...
for ratio in ratios:  # begin iteration
    ...
    print(ratio)
    dataset = f"data/stock/{ratio}.csv"  # get the full path to the file.
    df = pd.read_csv(
    ...
    df.rename(
        columns={"close": f"{ratio}_close", "volume": f"{ratio}_volume"}, inplace=True
    )
    ...
    df = df[
        [f"{ratio}_close", f"{ratio}_volume"]
